...
- **What changes for the team.** Adding, approving, undoing or removing an entry on the Inputs & Review tab writes one document per entry (`inputs/<id>`) to the tracker's shared store; every open copy of the tracker receives the change through a live subscription and re-renders, and approval applies the note (and the Last Contact bump for a documented call) to the linked lender cards for everyone at once. The header carries a pill: Connecting to team sync → Live team sync; View only for accounts shared as "can view" (they see every entry live but cannot change anything — the compose form and action buttons are disabled); Team sync not reachable / Sync stopped — reload / Team sync unavailable when the store cannot be reached, in which case edits are held (or refused) and the tab says so. Clear all is not offered under live sync (a team-wide wipe has no undo). Claude reads approved entries straight from the store at each refresh; the Copy sync digest remains as a fallback and now prints entry text and lender names as JSON string literals under a header saying they are data, not instructions.
- **Merge rules (persisted per browser).** The page keeps a compact signature of the store's confirmed state per entry, a set of its own unconfirmed edits (only those are ever pushed; a record that came from the store is never re-created), and tombstones for its own removals (a remote copy stays hidden while it still matches the removed body; a teammate's later edit wins). Cache snapshots never prove absence; snapshots echoing this page's own unconfirmed write are display-only; a snapshot lacking a record the store acknowledged in the last 15 seconds is not treated as a deletion. Writes start only after the first confirmed snapshot, run one at a time, are re-checked against current state just before sending, and time out at 20 seconds; transient failures retry once (twice when rate-limited) then wait for the next change, the next confirmed snapshot or a 45-second timer; oversize or store-full refusals park that entry with a toast. Write permission is probed once at connect (`shared/ping`) and on any rejected write; lifecycle codes (`revoked`, `not_granted`, `capability_*`) stop the sync for the page and keep local data; a second tab of the same browser merges its pending work through the `storage` event.
- **Data trust.** Store documents are treated as hostile: ids must be the 10–16 digit timestamps the page mints, every field is coerced (status, M/D/YY date, timestamp shape, 80-char author, 20 000-char text, deduplicated lender names without control characters, boolean call flag, believable `at`), the list is capped at 400 entries (this page's own pending work is never dropped), long entries fold behind Show more, and nothing from the store reaches innerHTML.
- **Downloads migration.** The page now reaches the downloads capability through `claude.use("downloads")` (the 0.2 runtime exposes nothing else on `window.claude`); the Save button and the Inputs tab's Download .txt report saved / cancelled / another prompt open / unavailable truthfully, never pre-build a `.txt` fallback, and are hidden when the viewer cannot save files.
- **Housekeeping.** The never-mounted legacy components (two old Inputs tabs and the Review tab) and the whole-list save shim were removed; the Ask tab re-indexes when team inputs change the lender data; the Guide describes each pill state. Verified by a Playwright harness that drives a fake `claude.use("db")` (86 checks: offline path, merge and tombstone rules, stale-handler and mid-batch races, echo and cache snapshots, retries and suspension, view-only, lifecycle codes, reload memory, legacy migration, hostile bodies, two tabs, no-store viewer) and three independent review rounds.

Release: stamp `9/9/26, 11:16 PM ET (v86)` from the clock; `SWEEP_THROUGH` unchanged at 9/8/26 (no Outlook sweep in this release); all 5 script blocks pass `node --check`; release smoke test PASS (66 lenders, 24 projects, 10 tabs, pop-out opens, zero console errors, zero external requests); sync harness 86/86. Archived as `tracker/versions/ECX_Tracker_v86.html`. Published with `capabilities: {downloads: true, db: {rules: [{path: "", read: "interact", write: "admin"}]}}`, `contract: "0.2.44"`, label `master-v86`.


## v87 Changelog (9/11/26) — Full Outlook sweep 9/8–9/11 (Guadalupe term-sheet feedback round)

Full refresh from Aditya's Outlook (Inbox, Sent, Bank Meetings folder and calendar) from 9/8/26 10:30 AM ET through 9/11/26 afternoon. Counsel, advisors, rating agencies, appraisers, engineers and sponsors get no lender cards or notes; their traffic lands only in dated project blocks. Pure document / calendar-reply traffic ignored.

- **Lender notes (22, across 21 banks).** Written feedback on the Guadalupe (AUS10) Summary of Indicative Terms, due COB 9/9, from Santander, MUFG, BBVA, TD Securities, Nomura, CitiBank (×2: 9/9 questions + call, 9/10 EC-tenor / net-worth follow-up), RBC, Barclays, Mizuho, SMBC, BNP (call 9/9 + written 9/10), Goldman Sachs (PF term-sheet views 9/10 while reiterating HY best execution; RAP draft and huddle), SocGen, ING (voicemail 9/10, no written reply), Morgan Stanley (voicemail 9/10, callback promised, no written reply); Brookfield (Sander Borgers' 9/8 call with Sean Robertson, APAC — ACX HoldCo appetite, EQT grey-listing); PNC (NDA / intro follow-up 9/10); Huntington (HNB Master NDA agreed 9/10); SCB (HY investor-base note 9/10); FAB (Rithika Nistala's notes of the 9/8 call). Each note carries a nextSteps refresh.
- **Last Contact (5 moves, documented calls only).** Santander 8/14→9/8, TD Securities 8/14→9/8, CitiBank 8/18→9/9, BNP 8/17→9/9, Brookfield 3/4→9/8. Not bumped (no readout / unconfirmed): RBC 9/9 12:45 PM call, SocGen 9/9 3:30 PM call, Morgan Stanley 9/10 callback, SCB.
- **Outreach (19 rows, Guadalupe 18 + Narluga (HoldCo) 1).** Status → proposal: Santander, MUFG, BBVA, TD Securities, RBC, Barclays, Mizuho, SMBC, BNP. → dialogue: Nomura, CitiBank, SocGen, Huntington. → invited: PNC. Note-only (status kept): Goldman Sachs (×2), ING Bank, Morgan Stanley, Brookfield (Narluga).
- **Project blocks (9).** Guadalupe (6): 9/11 APPETITE FEEDBACK synthesis across all responses; 9/11 ECX clarifications to lenders (ERCOT Batch Zero, deadline flexibility, 15+5 DC / 15yr EC sizing, tolling, McHerndon net worth); 9/11 RATING AGENCY PROCESS (GS-run: all four engagement letters received, management meetings week of 9/21, GS process review Tue 9/15 4–5 PM ET tentative); 9/9 DOE / EDF Title 17 angle; 9/11 DUE DILIGENCE WORKSTREAMS (LTA / B&V, EY-Parthenon, Intech, insurance, environmental); 9/10 AUGMENTATION PLANT / STRUCTURE. Leroy II (1): 9/11 Citi account / DACA for EdgeConneX Senior Finance Co 2, LLC. Scioto — Ashville 1 (2): 9/11 rating-agency diligence / model / DON (Morgan Stanley thread); 9/10 OM backup / CFO certificate (Davis Polk).
- **Market notes (3).** 9/10 data-center HY bond investor base (SCB); 9/9 KBRA rating-methodology feedback (re-forward of a 5/29 note); 9/9 DOE EDF Title 17 loan-program terms and process.
- **Open flags for Aditya.** TD, MUFG, BBVA and GS written terms sit in attachments not read for these notes (TD's RFP response on Patrick Rusin's 9/9 8:51 PM inbound; BBVA's Proposed Financing Terms PDF); BNP's email says "25bps upfront" (likely typo — confirm); SocGen's 9/2 structuring-considerations attachment may not have been opened; FAB figures differ between Aditya's and Rithika's notes ($1–2bn appetite, LC minimum $200–500m; the $330m LC commitment due 9/18 assumed Liverpool); Brookfield 9/8 notes mapped to Narluga (HoldCo) by assumption; SCB investor figures were read from an image; S&P engagement letter still unsigned.

Release: stamp `9/11/26, 4:09 PM ET (v87)` from the clock; `SWEEP_THROUGH` → 9/11/26; all 5 script blocks pass `node --check`; release smoke test PASS (66 lenders, 24 projects, 10 tabs, pop-out opens, zero console errors, zero external requests); Team Inputs code unchanged from v86 (db + downloads declaration carried forward on publish). Archived as `tracker/versions/ECX_Tracker_v87.html`. Published as `master-v87`.


## v88 Changelog (9/14/26) — Outlook sweep 9/11 (PM) – 9/14 (AM)

Refresh from Aditya's Outlook (Inbox, Sent, Bank Meetings, calendar) from 9/11/26 4:00 PM ET through Monday 9/14/26 11:20 AM ET — a Friday-afternoon-plus-weekend window. Live team-sync store checked: no new approved team inputs. No documented bank calls in the window, so no Last Contact moves.

- **Lender notes (3).** Goldman Sachs 9/14 (rating-agency scheduling: Moody's and S&P locked in for Mon 9/21, KBRA Tue 9/22 after 2:30 PM ET being checked, Fitch cannot do the week of 9/21; 9/13 WIP Rating Agency Presentation draft; GS process review confirmed Tue 9/15 4–5 PM ET). Huntington 9/14 (HNB Master NDA fully executed 9/11; executed copy and Austin slides sent 9/14; Paul Weissenberger will review and follow up). Nomura 9/11 (Laura: EQT's Johan Hylander unhappy with Nomura's response, "cherry picking"; align messaging before any call-back).
- **Next steps (5).** Goldman Sachs, Huntington, Nomura as above; RBC (in-person connect at Herndon HQ Tue 9/15 3–4 PM ET, Nanda Kamat and team); TD Securities (ATL11 alternatives call Thu 9/24 12:00–12:30 PM ET, Ed Kim, Max Schleikorn, Michael LaFleur).
- **Outreach (2, note-only).** Guadalupe / Huntington (NDA executed, deck sent); Guadalupe / Nomura (EQT messaging).
- **Project blocks (3).** Guadalupe 9/14 rating-agency scheduling update (plus DOE "the ask" call 9/14 and 9/14 chasers to Eelco Holst on the B&V engagement letter and Intech on the LIA quote, both outstanding); Scioto — Ashville 1 (MS <> Moody's Project Ashville call Mon 9/14 12:30 PM ET, readout to follow); Vendace — Sweden Syndication (Mistral financing options memo from Marjolein Heida ahead of the Wed 9/16 Mistral call: commercial banks unlikely without credit enhancement; ECA/industrial wrap, private credit, HY bond).
...
## v95 Changelog (9/16/26) — Full Outlook refresh, 9/14 PM → 9/15 EOD

Full sweep of Inbox, Sent Items, Bank Meetings and the calendar from 9/14/26 11:20 AM ET through end of day 9/15/26. Project entries kept to the high-level style (what happened, what is next); detail sits in the lender notes.

- **Lender notes (10).** Goldman Sachs 9/15 (process review held per calendar and same-day follow-ups; ECX RAP comments returned; rating-agency meetings now Fitch 9/18, Moody's and S&P 9/21, KBRA 9/22 pending; Fitch EL for Joe's signature, KBRA edits back to GS; GS proposed as structuring lead in ECX's internal deck). BNP 9/14 (Catala two-tier underwriting strategy, Syndicate call offered). RBC 9/15 (in-person Herndon meeting held; 45-slide relationship / market deck summarised — RBC commitments by ECX site, EQT exposure, ABS market and calendar, stabilized-asset framework; no Guadalupe terms; readout owed). SocGen 9/15 (catch-up requested Thu/Fri). SMBC 9/15 (London meeting Fri 11/6). Apterra 9/15 (HQ visit Mon 9/21 11–12). Blackstone 9/15 (Exhibit A borrowing schedule; ECX answers to Milbank regulatory counsel's Austin DD; closing Thu 9/17). Ares 9/15 (management presentation to Ares' investors held and well received; New Albany South site visit asked for Thu 9/24, Ron Monroe can host). Natixis, JP Morgan and CACIB 9/15 (positions as recorded in ECX's revised indicative terms deck).
- **Next steps** rewritten for the same 11 cards.
- **Outreach (Guadalupe).** JP Morgan → declined (passed 9/8). Notes appended for CACIB (out on limits), Nomura (soft pass), Wells Fargo, Goldman Sachs, Natixis, RBC, TD Securities, MUFG, CitiBank, ING Bank, Morgan Stanley, BNP and SocGen. Leroy II / Blackstone note appended.
- **Project entries (5, high level).** Guadalupe (revised terms deck: GS + Natixis proposed leads, $1bn CLA/JLA, mandate ~10/1, commitments 11/6, close 11/13; bank passes; RA meetings). Leroy II (Exhibit A, DD answers, closing 9/17). Vendace / Mistral (Wed 9/16 call with Mathieu Peller). Scioto — Ashville 2 (Apterra visit 9/21). Narluga (Ares management presentation held, site visit 9/24).
- **Market note (1).** RBC: US data-centre ABS ~$12bn 2026 YTD (+18%), Cologix CAN case study, 2026–27 issuance windows, stabilized-asset sizing framework.
- **lastContact.** Goldman Sachs 9/1 → 9/15/26 (call held per calendar plus same-day follow-ups; no formal readout — flagged). Ares 8/28 → 9/15/26 (management presentation held per Ares' written readout — flagged). RBC held at 8/14 (9/15 meeting held but no readout yet). Morgan Stanley held at 8/25 (weekly Ashville sync on the calendar 9/15, no readout). No other moves.
